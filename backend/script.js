...
function getDate(sec) {
    return (new Date(0, 0, 0, 0, sec / 60, (sec - 1) % 60));
}
...
function showGanttChart(output, outputDiv) {
    ...
    function drawGanttChart() {
        ...
        console.log(ganttChartData, dataTable, chart);
        let ganttWidth = '100%';
        if (startGantt >= 20) {
...
function showTimelineChart(output, outputDiv) {
    ...
    output.schedule.forEach((element) => {
        ...
        if (element[0] >= 0) { //process 
            console.log(getDate(startTimeline), getDate(startTimeline + element[1]));
...
function showFinalTable(input, output, outputDiv) {
    ...
    let el = tbt / lastct;
    cpu.innerHTML = "CPU Utilization : " + el.toFixed(2) * 100 + "%";
    outputDiv.appendChild(cpu);

    let tp = document.createElement("p");
    let th = process / lastct;
    tp.innerHTML = "Throughput : " + th.toFixed(2);
    outputDiv.appendChild(tp);

    let avgtat = 0;
    output.turnAroundTime.forEach((element) => (avgtat += element));
    avgtat = avgtat / process;
    let avgres = 0;
    output.responseTime.forEach((element) => (avgres += element));
    avgres = avgres / process;

    let avgt = document.createElement("p");
    avgt.innerHTML = "Average Turnaround Time = " + avgtat.toFixed(2);
    outputDiv.appendChild(avgt);

    let avgr = document.createElement("p");
    avgr.innerHTML = "Average Response Time = " + avgres.toFixed(2);
    outputDiv.appendChild(avgr);
...
function showAlgorithmChart(outputDiv) {
    let algochartdiv = document.createElement("div");
    algochartdiv.id = "algo-chart";
    // algochartdiv.style.height = (15 * process) + 300 + "px";
    let algoChartbutton = document.createElement("button");
    algoChartbutton.id = "show-algo-chart";
    algoChartbutton.innerHTML = "Comparision Chart";
    algochartdiv.appendChild(algoChartbutton);
    outputDiv.appendChild(algochartdiv);

    document.querySelector("#show-algo-chart").onclick = () => {
